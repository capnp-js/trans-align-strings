...
function browserRollup() {
  return rollup({
    ...
    bundle.write({
      file: "browser/capnp-js-trans-align-strings.js",
      format: "umd",
      name: "capnpJsTransAlignStrings",
      sourcemap: true,
      globals: {
        "@capnp-js/transform": "capnpJsTransform",
      },
    });
  });
}

//TODO: Should sourcemap reference before of after precompiling? After, I think.
function browserUglify() {
  return new Promise((resolve, reject) => {
    fs.readFile("browser/capnp-js-trans-align-strings.js.map", (err, map) => {
      if (err) {
        reject(err);
      } else {
        const ugly = gulp.src("browser/capnp-js-trans-align-strings.js")
          .pipe(uglify({
            sourceMap: {
              content: map.toString(),
              url: "capnp-js-trans-align-strings.js.map",
            },
          }))
